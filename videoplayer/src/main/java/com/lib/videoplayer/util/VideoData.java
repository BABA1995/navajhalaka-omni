...
public class VideoData {
    ...
    public static boolean isLandingVideoExist(Context aContext) {
        ...
        if (null != aContext) {
            ...
            String[] lSelectionArg = {VideoProvider.VIDEO_TYPE.TRAILER, VideoProvider.VIDEO_TYPE.COMEDY_SHOW, VideoProvider.VIDEO_TYPE.SERIAL, VideoProvider.VIDEO_TYPE.DEVOTIONAL, VideoProvider.VIDEO_TYPE.SPORTS, VideoProvider.VIDEO_TYPE.VIDEO_SONG, VideoProvider.DOWNLOAD_STATUS.DOWNLOADED};
            String orderBy = VideoProvider.VIDEO_COLUMNS.LAST_PLAYED_TIME + " ASC";
            Cursor lCursor = null;
            try {
                lCursor = aContext.getContentResolver().query(VideoProvider.CONTENT_URI_VIDEO_TABLE, null, lSelection, lSelectionArg, orderBy);
...
/*            if (isSingleTonType(data.getType())) {
    ...
    public static void deleteFile(String dFilePath) {
        if (null != dFilePath) {
            try {
                File deleteFile = new File(dFilePath);
                boolean status = deleteFile.delete();
                Log.e(TAG, "--(debug)-- :: File Delete status:: " + status);
            } catch (Exception e) {
        ...
        try {
            ...
            if (null != lCursor) {
                while (lCursor.moveToNext()) {
                    ...
                    movieCursor = VideoApplication.getVideoContext().getContentResolver().query(VideoProvider.CONTENT_URI_VIDEO_TABLE, null, lSelection, lSelectionArg, "name");
                    if (null != movieCursor) {
